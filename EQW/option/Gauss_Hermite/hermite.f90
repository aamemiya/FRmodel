...
call set_grid_hermite(ny,ylen,yl,yr,axy,wty)

w_taper=1.0e6
v_taper=1.0e7
do iy=1,ny
 taper(iy)=(0.5 + 0.5 * tanh( (v_taper-abs(axy(iy))) / w_taper ) ) 
end do
...
do ih=1,9
 ...
 ylens=ylen*fact_ylen/sqrt(real(2*ih+1))

do iy=1,ny
! f(iy)=exp(-(real(axy(iy))/(ylen*2.0))**2)*sin(3.1416/(ylen*3.6)*axy(iy))

 f(iy)=para_cyl(axy(iy)/ylens(ih),ih) * taper(iy)
! f(iy)=para_cyl(axy(iy)/ylen,ih)

 write(*,*) iy,axy(iy),f(iy)
end do

call decomp_fwd_gg(f,fh)
...
do ih=0,nh-1
 arrayout(ih+1)=0.0
 do iy=1,ny
  arrayout(ih+1) = arrayout(ih+1) + wty(iy) * para_cyl(axy(iy)/ylen,ih)*arrayin(iy) / ylen
 end do
end do
