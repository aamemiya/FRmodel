...
do ik=1,nx
 ...
 do ilayer=1,nlayer
  do j=1,2
   c_vm_bnd(ilayer,j)=fc_vm(bn2_bnd(ilayer,j),ubg_bnd(ilayer,j),axk(ik),vnu)
  end do
 end do
 ...
 do ilev=1,nlev
  if (ilev.eq.1) then
       c_phase(ilev) = fc_vm(bn2(ilev),ubg(ilev),axk(ik),vnu) * (zlev(ilev)-zbnds(inlay(ilev))) 
       c_phase_bnd(inlay(ilev))=0.0
  elseif (inlay(ilev).eq.inlay(ilev-1))then
       c_phase(ilev) = c_phase(ilev-1) + fc_vm(bn2(ilev-1),ubg(ilev-1),axk(ik),vnu) * (zlev(ilev)-zlev(ilev-1)) !! forward
  else
       c_phase(ilev) = fc_vm(bn2(ilev),ubg(ilev),axk(ik),vnu) * (zlev(ilev)-zbnds(inlay(ilev))) 
       c_phase_bnd(inlay(ilev)) = c_phase(ilev-1) + fc_vm(bn2(ilev-1),ubg(ilev-1),axk(ik),vnu) * (zbnds(inlay(ilev))-zlev(ilev-1)) !! forward
  end if
 end do
