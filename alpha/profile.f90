...
namelist /profile_nml/ p0,fname

open(11,file='calc.conf')
 read(11,nml=profile_nml)
close(11)
...
pread=pread*1.0e5 !!! sigma ==> Pa
!pread=pread*1.0e2 !!! hPa ==> Pa
...
!zbnds=(/zbot,zbnd,ztop/) !!! TORI AEZU
!zbnds=(/zbot,zbnd,ztop/) !!! TORI AEZU
zbnds=(/zbot,zbnd,ztop/) !!! TORI AEZU
!zbnds=(/zbot,ztop/) !!! TORI AEZU

ubg_bnd(1,1:2) = ubg(1)
vbg_bnd(1,1:2) = vbg(1)
tbg_bnd(1,1:2) = tbg(1)
bn2_bnd(1,1:2) = bn2(1)
ubg_bnd(2,1:2) = ubg(ibnd:ibnd+1)
vbg_bnd(2,1:2) = vbg(ibnd:ibnd+1)
tbg_bnd(2,1:2) = tbg(ibnd:ibnd+1)
bn2_bnd(2,1:2) = bn2(ibnd:ibnd+1)
 !!! 
...
!      call uulin  (nr,vm2smp(1:nr),zread(1:nr))
      ...
      call sgtxzv (0.45,0.75,'sample profile N|2"',0.020,0,0,5) ! title
