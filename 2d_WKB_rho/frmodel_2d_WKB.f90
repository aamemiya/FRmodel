...
do ik=1,nx
 c_phase=0.0
 c_vm0=fc_vm(bn2(1),ubg(1),axk(ik),vnu,tbg(1))
 ilev = 1
   c_amp=c_hk(ik)
   c_emzhk(ik,ilev) = c_amp * exp( c_ui * c_phase )
   call plrz_diag(axk(ik),ubg(ilev),bn2(ilev),c_vm0,c_emzhk(ik,ilev),c_vx(ik,ilev),c_vu(ik,ilev),c_vw(ik,ilev),vnu,tbg(ilev))
 do ilev=2,nlev
...
!=============================================!
!!! polarization relation with density factor correction
...
!!! fc_vm =  ( ( vbn2 / vu**2 - vk**2 ) * c_ur ) ** 0.5
 ...
 fc_vm =  ( vk**2 * ( vbn2 * c_ur - c_oh**2 ) / c_oh**2 - alpha**2 ) ** 0.5
